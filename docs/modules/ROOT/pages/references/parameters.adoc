...
The parent key for all of the following parameters is `openbao`.
...
Two intances of OpenBao can be deployed in the same namespace, but the two instances will have access to each others unseal secrets.
...
The default chart version used by the component requires Kubernetes 1.30 or newer.
...
Container image configuration for the OpenBao server.
...
Container image configuration for the `bank-vaults` unsealer sidecar.
...
The `podManagementPolicy` for the OpenBao StatefulSet
...
This section allows users to configure how OpenBao uses the information in the `X-Forwarded-For` header in client connections.
...
Since OpenBao doesn't accept the empty string as a valid option, we set the parameter to only trust `X-Forwarded-For` headers from `127.0.0.1/32` by default.

To avoid issues with parameter interpolation, multiple entries should be specified as a comma-separated list.

If you want to use functionality in OpenBao which requires the real source IP of requests, you should set this parameter to a CIDR which includes the IPs of your ingress controller.

Also see the https://openbao.org/docs/configuration/listener/tcp/#x_forwarded_for_authorized_addrs[OpenBao documentation].
...
See the https://openbao.org/docs/configuration/listener/tcp/#x_forwarded_for_hop_skips[OpenBao documentation] for more details.
...
[horizontal]
type:: dict
default:: https://github.com/containeroo/component-openbao/blob/master/class/defaults.yml[See `class/defaults.yml`]

The configuration for OpenBao.
...
auth_:
  ldap:
    ...
    config:
      ...
      binddn: "uid=openbao-service,ou=Users,dc=todo,dc=com"
      bindpass: ?{vaultkv:${cluster:tenant}/${cluster:name}/openbao/ldap/password}
      userattr: uid
      userdn: "ou=openbao,ou=Service Access,ou=Views,dc=todo,dc=com"
      groupdn: "ou=Groups,dc=todo,dc=com"
      groupattr: cn
    groups:
      OpenBao root:
        policies: openbao-root
----
...
auth_:
  oidc:
    ...
    config:
      ...
      oidc_client_secret: ?{vaultkv:${cluster:tenant}/${cluster:name}/openbao/${_instance}/oidc/client_secret}
...
[horizontal]
type:: string
default:: `?{vaultkv:${cluster:tenant}/${cluster:name}/openbao/backup/password}`
